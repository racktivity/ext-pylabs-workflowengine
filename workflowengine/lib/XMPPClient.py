...
class XMPPClient:
    ...
    def start(self):
        ...
        q.logger.log("[SL XMPPCLIENT] Starting the xmpp client for " + self.__username + "@" + self.__server, 3)
        try:
            self._connect()
        except Exception, e:
    ...
    def stop(self):
        ...
        q.logger.log("[SL XMPPCLIENT] Stopping the xmpp client for " + self.__username + "@" + self.__server, 3)
        self._close()
    ...
    def _reconnect(self):
        while not self.__connected:
            ...
            q.logger.log("[SL XMPPCLIENT] Reconnecting " + self.__username + "@" + self.__server + " in " + str(self.__delay) + " seconds.", 3)
    ...
    def _connect(self):
        ...
        self.stream.write_start(self.__server)
        
        #perform auth handshake
        self._handshake(self.__username, self.__password, self.__server)
        
        q.logger.log("[SL XMPPCLIENT] Server '" + self.__server + "' authenticated user '" + self.__username + "'", 5)
    ...
    def _handshake(self, username, password, realm):
        ...
        if element.tag == '{urn:ietf:params:xml:ns:xmpp-sasl}failure':
            q.logger.log("[SL XMPPCLIENT] Authentication of : " + self.__username + " on " + self.__server + " failed.", 3)
    ...
    def sendMessage(self, to, type, id, message=' '):
        ...
        @type message: string
        '''
        q.logger.log("[SL XMPPCLIENT] Sending message '" + str(id) + "' of type '" + str(type) +"' to " + str(to) + " for " + self.__username + "@" + self.__server, 5)
        self.__queueTillConnected()
        self.stream.write_message(to+"@"+self.__server, type, id, message)
            ...
            try:
                for element in self.__elements:
                    if element.tag == '{jabber:client}presence':
                        ...
                        q.logger.log("[SL XMPPCLIENT] Received presence from '" + fromm + "' of type '" + type + " for " + self.__username + "@" + self.__server, 5)
                        ...
                        q.logger.log("[SL XMPPCLIENT] Received message '" + element.get('id') + "' from '" + fromm + "' of type '" + element.get('type') + " for " + self.__username + "@" + self.__server, 5)
                        yield {'type':'message', 'from':fromm, 'message_type':element.get('type'), 'id':element.get('id'), 'message':message}
                    else:
                        q.logger.log("[SL XMPPCLIENT] Received wrong tag: '" + str(element.tag)  + " for " + self.__username + "@" + self.__server, 5)
            except EOFError:
