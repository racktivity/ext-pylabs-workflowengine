...
class DRPTask:
    ...
    def connectDRPClient(self, drpClient):
        ...
        alog('connectDRPClient from osis PRE')
        from osis import ROOTOBJECT_TYPES as types
        alog('connectDRPClient from osis POST')
        alog('connectDRPClient type itervalues %r'% types.itervalues())
        for type in types.itervalues():
            alog('connectDRPClient for type %r'%type)
            name = getattr(type, 'OSIS_TYPE_NAME', type.__name__.lower())
            alog('connectDRPClient for type name %r'%name)
            if name in self.bufferedObjects:
                buffer = {}
                self.__buffers[name] = buffer
                alog('drp setattr %r'%[name,self,buffer])
